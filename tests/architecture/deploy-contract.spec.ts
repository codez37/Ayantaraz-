...
const STATE_DIR = join(DEPLOY_DIR, 'state');
const ENV_DIR = join(DEPLOY_DIR, 'env');
...
describe('CONTRACT 4: Lock Exclusivity', () => {
  it('acquire_lock uses flock -n (non-blocking)', () => {
    const helper = readFileSync(join(__dirname, '../../scripts/helper-part2.sh'), 'utf8');
...
describe('CONTRACT 5: Gate Stage Enforcement', () => {
  const HELPER_PATH = join(__dirname, '../../scripts/helper-part3.sh');
...
describe('CONTRACT 7: Graceful Drain', () => {
  it('drain sends nginx -s quit', () => {
    const helper = readFileSync(join(__dirname, '../../scripts/helper-part3.sh'), 'utf8');
...
describe('CONTRACT 14: Bootstrap Safety', () => {
  it('bootstrap.sh checks for existing env before creating', () => {
    const bootstrap = readFileSync(join(__dirname, '../../scripts/bootstrap.sh'), 'utf8');
...
describe('CONTRACT 15: Dependency Completeness', () => {
  it('bootstrap installs all tools used by deploy helper', () => {
    const bootstrap = readFileSync(join(__dirname, '../../scripts/bootstrap.sh'), 'utf8');
    const helper1 = readFileSync(join(__dirname, '../../scripts/helper-part1.sh'), 'utf8');
    const helper2 = readFileSync(join(__dirname, '../../scripts/helper-part2.sh'), 'utf8');
    const helper3 = readFileSync(join(__dirname, '../../scripts/helper-part3.sh'), 'utf8');

    const allHelpers = helper1 + helper2 + helper3;

    // Tools that deploy helper uses and bootstrap must install
    const REQUIRED_TOOLS = ['flock', 'openssl', 'curl', 'jq', 'grep', 'awk', 'sort', 'tail', 'wc'];

    for (const tool of REQUIRED_TOOLS) {
      if (allHelpers.includes(tool + ' ')) {
        // Only check if actually used as a command (not in strings)
        const isUsedAsCommand = allHelpers.includes(`${tool} `) || allHelpers.includes(`"${tool}"`);
        if (isUsedAsCommand) {
          const installed = bootstrap.includes(tool);
          // Not all tools need explicit install (some are bash builtins)
          if (!['sort', 'tail', 'wc', 'grep', 'awk'].includes(tool)) {
